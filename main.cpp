...
void triangle(TGAImage &image,Vec3i t0, Vec3i t1, Vec3i t2, int *zbuffer, float intensity,Vec2i uv0, Vec2i uv1, Vec2i uv2){	
...
int main(int argc, char** argv) {	
	...
	for( int i=0; i<model->nfaces(); i++){
		...
		if(intensity>0){
			...
			triangle(image, screen_coords[0], screen_coords[1], screen_coords[2],zbuffer,intensity,uv[0],uv[1],uv[2]);
		}
	}
	...
	for(int i = 0; i< width; i++){
		for(int j=0; j<height; j++){
			zImage.set(i,j, TGAColor(zbuffer[i+j*width],1));
		}
	}
